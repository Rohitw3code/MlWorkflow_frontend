...
function DataEncoding() {
  ...
  const fetchData = async () => {
    try {
      const resp = await fetch(`${apiurl}/api/encode-columns`);
      if (resp.ok) {
  ...
  const fetchColsData = async () => {
    const url = `${apiurl}/api/df/colsdata`; // Replace with your API endpoint URL
  ...
  const encodeDF = async () => {
    const url = `${apiurl}/api/df/encode-df`; // Replace with your API endpoint URL
    // Data to be sent in the request body
    const payload = {
      cols: selectedColumns,
    };
      ...
      >
        ...
        {columns.map((item, index) => (
          <button
            ...
            ${color === 'aurora--bgr' && 'bg-white/70 text-gray-900'}`}
            key={index}
            onClick={() => {
          ...
          {selectedColumns.length > 0 ? (
            <>
              <button
                ...
                ${color === 'gradient--black' && 'bg-slate-700'} ${color === 'aurora--bgr' && 'bg-white/70 text-gray-800'}`}
                onClick={() => {
                  encodeDF();
                }}
              >
